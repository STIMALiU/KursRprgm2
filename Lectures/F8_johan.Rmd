...
+===============+===============+========================+
| `str_sub()`   | `substr`      | substring, välj ut en av |
|               |               | en sträng (regex)      |
+---------------+---------------+--------------------+
| `str_c()`     | `paste()`     | Slår ihop strängar   |
|               | `paste0()`    |                    |
+---------------+---------------+--------------------+
| `str_split()` | `strsplit()`  | Dela upp en sträng i flera element (regex)   |
|               |               |                    |
+---------------+---------------+--------------------+
| `str_length()`| `nchar()`     | Antal tecken       |
|               |               |                    |
+---------------+---------------+--------------------+
| `str_trim()`  |               | Tar bort mellanslag   |
|               |               |                    |
+---------------+---------------+--------------------+
| `str_pad()`   |               | Lägger till mellanslag   |
|               |               |                    |
+---------------+---------------+--------------------+
...
| `stringr`           | `base`        | Användning             |
+=====================+===============+========================+
| `str_detect()`      | `grepl()`     | Identifiera `pattern`, returnera logisk vektor |
+---------------------+---------------+--------------------+
| `str_locate()`      | `gregexpr()`  | Identifiera `pattern`, returnera position i texten |
+---------------------+---------------+--------------------+
| `str_replace()`     | `gsub()`      | Identifiera `pattern`, ersätter med ny text |
+---------------------+---------------+--------------------+
